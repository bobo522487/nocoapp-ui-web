...
import { Info, ArrowRight, Settings2, X, Database, ShieldCheck, Globe, Save, Trash2 } from 'lucide-react';
...
interface EditColumnDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  initialField: SchemaField | null;
...
const EditColumnDrawer: React.FC<EditColumnDrawerProps> = ({ 
    isOpen, 
    onClose, 
    initialField,
    onSave, 
  ...
  useEffect(() => {
    if (isOpen && initialField) {
        ...
        setColumnName(initialField.name);
        setDataType(initialField.type);
        setDefaultValue(initialField.defaultValue || '');
        setTimeZone(initialField.timeZone || 'UTC');
        
        setIsNotNull(!initialField.isNullable);
        setIsUnique(initialField.isUnique);
        
        const fkConf = (initialField as any).foreignKeyConfig;
        setIsForeignKey(initialField.isForeignKey && !!fkConf);
        setFkConfig(fkConf || null);
  ...
  const handleSaveWrapper = () => {
      if (!columnName.trim() || !initialField) return;
      ...
      const updatedField: SchemaField = {
          ...
          isUnique,
          // Maintain original primary key status unless explicitly changed (not exposed in UI yet for editing to prevent breaking PKs easily)
          // isPrimary: dataType === 'serial' ? true : initialField.isPrimary, 
          timeZone: dataType === 'date with time' ? timeZone : undefined,
          icon: DATA_TYPES.find(t => t.id === dataType)?.icon || Type
      };

      if (isForeignKey && fkConfig) {
          (updatedField as any).foreignKeyConfig = fkConfig;
      } else {
          delete (updatedField as any).foreignKeyConfig;
      }
